...
class Gamma:
    ...
    def __init__(self, *args):
        ...
        # Multipolarity info - not sure how to parse this yet
    ...
    def __str__(self):
        return dedent(f"""\
            Gamma Ray Object.
                Energy:          {self.gE} +/- {self.gE_err}
                Rel. Intensity:  {self.RI}
                Conv. Coeff.:    {self.CC}
                Parent Level:    {self.ExT} spin {self.SpnT}
                Child Level:     {self.ExB} spin {self.SpnB}
                Branching Ratio: {self.BR:.3f}""")
    ...
    def is_valid(self):
        """Basic sanity checks to determine if Gamma was constructed correctly.
        
        Not an exhaustive set of criteria.
        """
        valid = True
        valid = valid and (self.gE > 0)
        valid = valid and (self.RI > 0)
        valid = valid and (self.ExT > 0)
        return valid
...
class Level:
    ...
    def __init__(self, exE):
        """Construct an energy level with a given energy.
        
        Energy levels are always created without incoming 
        or outgoing gamma rays."""
        self.ExE = exE
        self.outGammas = []   # List of gamma rays emitted from this level
        self.inGammas=[]      # List of gamma rays that directly populate this level
    ...
    def add_outgoing_gamma(self, gamma):
        """Add a gamma ray to the list of outgoing rays."""
        self.outGammas.append(gamma)
    
    def add_incoming_gamma(self, gamma):
        """Add a gamma ray to the list of incoming rays."""
        self.inGammas.append(gamma)

    def compute_BRs(self):
        """Set the branching ratio for each outgoing gamma ray.
        
        The branching ratio for an outgoing gamma ray is the 
        relative intensity of that ray divided by the total 
        relative intensity among all outgoing rays."""
        total = 0
    ...
    def list_gammas(self):
        """Print all outgoing gamma rays from this level."""
        for gamma in self.outGammas:
...
class LevelScheme:
    ...
    def get_energies(self):
        """Return list of excitation energies."""
    ...
    def add_level(self, lvl):
        """Add energy level to graph and update corresponding class members."""
    ...
    def map_from_transition_space(self,TS):
        ...
        for edge in TS.g.edges():
            # Scenario #1: Two consecutive gamma rays, neither of which is in the level scheme yet.
            #              Make three new levels.
            if edge[0] not in self.placed and edge[1] not in self.placed:
                # Add lowest level
                newLvl=Level(0.1)
            ...
            # Scenario #2: Incoming gamma ray is in level scheme, but outgoing is not
            ...
            # Scenario #3: Incoming gamma ray is not in level scheme, but outgoing is
            ...
            # Scenario #4: Both gamma rays are already in level scheme
            else:
                for lvl in self.levels:
        ...
        self.connect_nodes_with_edges()
        self.delete_edges_and_nodes()
    
    def connect_nodes_with_edges(self):
        """Add edges between all excitation levels."""
...
class TransitionScheme:
    ...
    def __init__(self, verbose = 0):
        """Create an empty Transition Scheme."""
        ...
        self.gammaRays = []     # List of all gamma-ray objects
    ...
    def build_from_adjacency_matrix(self, adjMatrix, intensity_threshold, gammas):
        """Assembles scheme from a transition-centric adjacency matrix and list of gamma objects.
        
        This method constructs the underlying graph representation of the scheme,
        and also updates other class members. In particular, it constructs:
            * g
            * adjDict
            * leafNodes
            * branchNodes
            * gammaRays
        It DOES NOT construct: nt, nodeDict, allPaths, verbose."""
        nrows = adjMatrix.shape[0]
        ncols = adjMatrix.shape[1] # n.b. this should be equal to nrows
        self.gammaRays = gammas
        
        # Consider each row of adjMatrix and add nodes/edges if non-negligible entries are found
        for i in range(nrows):
            isLeaf = True
            gE = gammas[i].gE
            self.adjDict.update({gE:[]}) # Add gamma energy to dictionary of adjacent transitions
            self.g.add_node(gammas[i].gE, Gamma=gammas[i]) # Add node to graph
            for j in range(ncols):
                if adjMatrix[i][j] > intensity_threshold:
                    isLeaf = False
            ...
            # Update lists of leaf and branch nodes
            if isLeaf:
    ...
    def find_all_paths(self,source,destination):
        """Use depth first search with backtracking to find all paths in the graph."""
    ...
    def printPaths(self):
        """Print a list of all paths through the graph that have been found so far."""
        print("Printing all paths through the transition scheme:")
